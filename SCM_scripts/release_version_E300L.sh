...
cp $rootDir/$AMSS_Path/TZ.BF.4.0.5/trustzone_images/core/bsp/qsee/build/ZALAANAA/qsee.elf amss_symbols/TZ.BF.4.0.5_WW/trustzone_images/core/bsp/qsee/build/ZALAANAA

mkdir -p amss_symbols/ADSP.8953.2.8.4/adsp_proc/build/ms
mkdir -p amss_symbols/ADSP.8953.2.8.4/adsp_proc/qdsp6/qshrink/src
cp $rootDir/$AMSS_Path/ADSP.8953.2.8.4/adsp_proc/build/ms/*.elf amss_symbols/ADSP.8953.2.8.4_WW/adsp_proc/build/ms/
cp $rootDir/$AMSS_Path/ADSP.8953.2.8.4/adsp_proc/qdsp6/qshrink/src/msg_hash.txt amss_symbols/ADSP.8953.2.8.4_WW/adsp_proc/qdsp6/qshrink/src/
fi
...
if [ -d $Overall_all_images ];then
    ...
    cd $OUT_Path
    if [ -f "${VER_NO}_DL.zip" ]; then
	rm -rf "${VER_NO}_DL.zip"
    fi
...
#    rm -rf overall_all_images.tar.gz
    PRODUCT_NAME=$(echo $PRODUUCT_NAME | awk -F '_' '{print $1}')
    if [[ "$xtt_download" != "yes" && "$PRODUCT_NAME" == "E300L" ]]
    then
        git clone ssh://10.0.30.9:29418/SCM_script
        mv SCM_script/fastboot/E300L/* overall_all_images/
        rm -rf SCM_script
    fi
    mv overall_all_images "${VER_NO}_DL"
    zip -r "${VER_NO}_DL".zip "${VER_NO}_DL"/*
    mv "${VER_NO}_DL"/ overall_all_images/
    
    FILES=$FILES" "$OUT_Path"/""${VER_NO}_DL".zip
    filesize=`ls -lk "${VER_NO}_DL".zip | awk '{print $5}'`
    echo_greeen "${VER_NO}_DL.zip ---- $filesize Bytes"
    md5=`md5sum -b "${VER_NO}_DL".zip`
    if [ -f "$OUT_Path/checklist.md5" ]; then
        line=`grep -n "${VER_NO}_DL.zip" $OUT_Path/checklist.md5 | cut -d ":" -f 1`
    fi
    if [ x"$line" != x"" ]; then
        sed -i $line's/.*/'"$md5"'/' $OUT_Path/checklist.md5
    else
